...
def get_product(ctx: AuthContext, product_id: int) -> dict[str, Any]:
    """Look up one public catalog product by its identifier. Risk tier: read.

    Product catalog data is public to every Cartwheel role. Return the stable
    identifier, store, title, category, and price; return a structured
    ``not_found`` error when the identifier is unknown.
    """
    with db.connection() as conn:
        product = next(
            (candidate for candidate in db.list_products(conn) if candidate.id == product_id),
            None,
        )
    if product is None:
        return {
            "ok": False,
            "error": "not_found",
            "reason": f"no product with id {product_id}",
        }
    return {
        "ok": True,
...
def search_products(
    ...
    with db.connection() as conn:
        ...
        if store is not None:
            store_record = db.get_store_by_name(conn, store)
        ...
        candidates = db.list_products(
            conn, store_record.id if store_record is not None else None
        )
...
def list_my_orders(ctx: AuthContext) -> dict[str, Any]:
    ...
    with db.connection() as conn:
        ...
        else:  # AuthContext guarantees that a merchant has a store_id.
...
def find_order(ctx: AuthContext, query: str) -> dict[str, Any]:
    ...
    if ctx.role == "shopper":
        if ctx.user_id is None:
            return {
                "ok": False,
                "error": "invalid_argument",
                "reason": "shopper order search requires a user id",
            }
        ...
        if ctx.store_id is None:
            return {
                "ok": False,
                "error": "invalid_argument",
                "reason": "merchant order search requires a store id",
            }
    ...
    with db.connection() as conn:
        ...
        product_titles = {
            product.id: product.title.lower() for product in db.list_products(conn)
        }

    matches = []
    for order in candidates:
        title = product_titles.get(order.product_id, "")
        if title and fuzz.partial_ratio(title, query) >= ORDER_MATCH_THRESHOLD:
